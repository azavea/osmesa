...
KEY_PAIR_NAME=$(cat ${TF_STATE_FILE} | jq -r ".modules[].resources[\"aws_emr_cluster.emrSparkCluster\"].primary.attributes[\"ec2_attributes.0.key_name\"]")
KEY_PAIR_FILE="~/.ssh/${KEY_PAIR_NAME}.pem"
MASTER_DNS=$(cat ${TF_STATE_FILE} | jq '.modules[0].resources["aws_emr_cluster.emrSparkCluster"].primary.attributes.master_public_dns')
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
    if [ "${1:-}" = "--help" ]; then
        ...
        case "${1}" in
            install-geomesa)
                cat "${DIR}/../deployment/terraform/geomesa-bootstrap.sh" | \
            ...
            proxy)
                ...
                ;;
            *)
                echo "We don't support the command ${1}";
                usage;
                exit 1
                ;;
        esac
