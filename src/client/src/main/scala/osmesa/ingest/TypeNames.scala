...
object TypeNames extends CommandApp(
  name = "List Type Names in an OSMesa table",
  header = "List Type Names in an OSMesa table",
  main = {
    ...
    (datastoreConfO, localF).mapN({ (dsConfUri, local) =>
      ...
      try {
        Util.loadYamlAsDatastoreConf(dsConfUri).map({ dsConf =>
          ...
          case Valid(names) =>
            println("Types detected in DataStore:")
